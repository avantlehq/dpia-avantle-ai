...
export class SystemService {
  ...
  async updateSystem(id: UUID, data: UpdateSystemRequest): Promise<System> {
    ...
    const validationData = {
      ...existingSystem,
      ...data,
      description: data.description ?? existingSystem.description,
      system_type: data.system_type ?? existingSystem.system_type,
      owner_team: data.owner_team ?? existingSystem.owner_team,
  ...
  private assessSystemSecurityCompliance(securityOverview: { security_score: number }): { score: number; issues: string[] } {
    const issues: string[] = [];
