-- Apply all Context Module migrations in sequence - FIXED VERSION
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE event_object_table = 'jurisdictions' 
        AND trigger_name = 'update_jurisdictions_updated_at'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'physical_locations' 
        AND trigger_name = 'update_physical_locations_updated_at'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'systems' 
        AND trigger_name = 'update_systems_updated_at'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'data_categories' 
        AND trigger_name = 'update_data_categories_updated_at'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'processing_activities' 
        AND trigger_name = 'update_processing_activities_updated_at'
    ) THEN
...
RETURNS UUID AS $$
BEGIN
  RETURN (current_setting('request.jwt.claims', true)::json->>'workspace_id')::uuid;
EXCEPTION
  WHEN OTHERS THEN
    RETURN '00000000-0000-0000-0000-000000000001'::uuid; -- Default workspace for service role
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Policies for jurisdictions
DROP POLICY IF EXISTS "Users can view jurisdictions in their workspace" ON jurisdictions;
...
DROP POLICY IF EXISTS "Users can insert jurisdictions in their workspace" ON jurisdictions;
...
DROP POLICY IF EXISTS "Users can update jurisdictions in their workspace" ON jurisdictions;
CREATE POLICY "Users can update jurisdictions in their workspace"
  ON jurisdictions FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete jurisdictions in their workspace" ON jurisdictions;
...
DROP POLICY IF EXISTS "Users can view physical locations in their workspace" ON physical_locations;
CREATE POLICY "Users can view physical locations in their workspace"
  ON physical_locations FOR SELECT
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can insert physical locations in their workspace" ON physical_locations;
CREATE POLICY "Users can insert physical locations in their workspace"
  ON physical_locations FOR INSERT
  WITH CHECK (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can update physical locations in their workspace" ON physical_locations;
CREATE POLICY "Users can update physical locations in their workspace"
  ON physical_locations FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete physical locations in their workspace" ON physical_locations;
CREATE POLICY "Users can delete physical locations in their workspace"
  ON physical_locations FOR DELETE
  USING (workspace_id = get_workspace_id());

-- Policies for vendors
DROP POLICY IF EXISTS "Users can view vendors in their workspace" ON vendors;
CREATE POLICY "Users can view vendors in their workspace"
  ON vendors FOR SELECT
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can insert vendors in their workspace" ON vendors;
CREATE POLICY "Users can insert vendors in their workspace"
  ON vendors FOR INSERT
  WITH CHECK (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can update vendors in their workspace" ON vendors;
CREATE POLICY "Users can update vendors in their workspace"
  ON vendors FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete vendors in their workspace" ON vendors;
CREATE POLICY "Users can delete vendors in their workspace"
  ON vendors FOR DELETE
  USING (workspace_id = get_workspace_id());

-- Policies for systems
DROP POLICY IF EXISTS "Users can view systems in their workspace" ON systems;
CREATE POLICY "Users can view systems in their workspace"
  ON systems FOR SELECT
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can insert systems in their workspace" ON systems;
CREATE POLICY "Users can insert systems in their workspace"
  ON systems FOR INSERT
  WITH CHECK (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can update systems in their workspace" ON systems;
CREATE POLICY "Users can update systems in their workspace"
  ON systems FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete systems in their workspace" ON systems;
CREATE POLICY "Users can delete systems in their workspace"
  ON systems FOR DELETE
  USING (workspace_id = get_workspace_id());

-- Policies for data_categories
DROP POLICY IF EXISTS "Users can view data categories in their workspace" ON data_categories;
CREATE POLICY "Users can view data categories in their workspace"
  ON data_categories FOR SELECT
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can insert data categories in their workspace" ON data_categories;
CREATE POLICY "Users can insert data categories in their workspace"
  ON data_categories FOR INSERT
  WITH CHECK (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can update data categories in their workspace" ON data_categories;
CREATE POLICY "Users can update data categories in their workspace"
  ON data_categories FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete data categories in their workspace" ON data_categories;
...
DROP POLICY IF EXISTS "Users can view processing activities in their workspace" ON processing_activities;
CREATE POLICY "Users can view processing activities in their workspace"
  ON processing_activities FOR SELECT
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can insert processing activities in their workspace" ON processing_activities;
CREATE POLICY "Users can insert processing activities in their workspace"
  ON processing_activities FOR INSERT
  WITH CHECK (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can update processing activities in their workspace" ON processing_activities;
CREATE POLICY "Users can update processing activities in their workspace"
  ON processing_activities FOR UPDATE
  USING (workspace_id = get_workspace_id());

DROP POLICY IF EXISTS "Users can delete processing activities in their workspace" ON processing_activities;
