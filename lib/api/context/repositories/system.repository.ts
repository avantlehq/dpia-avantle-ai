...
> {
  ...
  async advancedSearch(filters: {
    ...
    if (filters.has_endpoints !== undefined) {
      if (filters.has_endpoints) {
        ...
        if (systemsWithEndpoints?.length) {
          const systemIds = [...new Set(systemsWithEndpoints.map(e => e.system_id))];
    ...
    if (filters.locations?.length) {
      ...
      if (systemLocations?.length) {
        const systemIds = [...new Set(systemLocations.map(sl => sl.system_id))];
        query = query.in('id', systemIds);
      }
    }
  ...
  async getSecurityOverview(systemId: UUID): Promise<{
    ...
    const encryptedInTransit = endpoints.filter(e => e.encryption_in_transit).length;
    const encryptedAtRest = endpoints.filter(e => e.encryption_at_rest).length;
    const authenticated = endpoints.filter(e => e.authentication_method).length;
